    ...
    <!-- {{ $history->id }} -->
      ...
      <tr class="">
        ...
        <td class="p-4 border-b border-blue-gray-50">
          ...
          <!-- button delete -->
          <form action="/duitmasuk" method="post">
            @csrf
            @method('DELETE')
            <input type="hidden" name="id" value="{{ $history->id }}">
            <button class="relative align-middle select-none font-sans font-medium text-center uppercase transition-all disabled:opacity-50 disabled:shadow-none disabled:pointer-events-none w-10 max-w-[40px] h-10 max-h-[40px] rounded-lg text-xs text-blue-gray-500 hover:bg-blue-gray-500/10 active:bg-blue-gray-500/30" type="submit">
              <span class="absolute top-1/2 left-1/2 transform -translate-y-1/2 -translate-x-1/2" onclick="return alert('apakah anda yakin ingin menghapus?')">
